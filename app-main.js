...
(() => {
  ...
  /* =====================================================================
     FONDO HACKER / MATRIX ANIMADO
     ===================================================================== */
  let matrixActivo = false;

  function instalarFondoMatrix() {
    if (matrixActivo) return;
    matrixActivo = true;

    const canvas = document.createElement("canvas");
    canvas.id = "rp-matrix-canvas";
    canvas.setAttribute("aria-hidden", "true");
    document.body.prepend(canvas);

    const ctx = canvas.getContext("2d");
    const chars = "0101011010010110ROPROSTPREDICTIA0123456789";
    let columnas = [];
    let ancho = 0;
    let alto = 0;
    const fontSize = 18;
    ...
    function ajustar() {
      ...
      const total = Math.ceil(ancho / fontSize);
      columnas = Array.from({ length: total }, () => Math.floor(Math.random() * alto / fontSize));
    }

    function pintar() {
      ctx.fillStyle = "rgba(0, 5, 2, 0.085)";
      ctx.fillRect(0, 0, ancho, alto);
      ctx.font = fontSize + "px monospace";

      for (let i = 0; i < columnas.length; i++) {
        const texto = chars.charAt(Math.floor(Math.random() * chars.length));
        const x = i * fontSize;
        const y = columnas[i] * fontSize;

        ctx.fillStyle = Math.random() > 0.965 ? "rgba(220, 255, 230, 0.95)" : "rgba(74, 222, 128, 0.58)";
        ctx.shadowColor = "rgba(74, 222, 128, 0.85)";
        ctx.shadowBlur = 10;
        ctx.fillText(texto, x, y);
        ctx.shadowBlur = 0;

        if (y > alto && Math.random() > 0.976) columnas[i] = 0;
        columnas[i]++;
      }
  ...
  /* =====================================================================
     SEGURIDAD DE ENTRADA CON FIREBASE AUTH
     ===================================================================== */
  const ADMIN_USER = "admi";
  const ADMIN_EMAIL = "admi@roprost.local";
  const ADMIN_UID = "26xq34oz9tf1gAYGKjs9EKlxVsF2";

  const firebaseConfig = {
    apiKey: "AIzaSyDfV9gVRvsKqpazibGLGYmW1B4YdPVBy9Y",
    authDomain: "adminflujo.firebaseapp.com",
    projectId: "adminflujo",
    storageBucket: "adminflujo.firebasestorage.app",
    messagingSenderId: "408780728879",
    appId: "1:408780728879:web:a9adb2cb68c8f14482b4da"
  };
  ...
  function iniciarFirebaseAuth() {
    ...
    if (!window.firebase || !window.firebase.auth) {
      console.error("Firebase Auth no está cargado. Revisa index.html.");
      return false;
    }
    ...
    try {
      ...
      fbAuthReady = new Promise(function(resolve) {
  ...
  async function usuarioFirebaseActual() {
    if (!iniciarFirebaseAuth()) return null;
    if (fbAuth.currentUser) return fbAuth.currentUser;
    if (fbAuthReady) await fbAuthReady;
    return fbAuth.currentUser || null;
  }

  function esAdminFirebase(user) {
    return !!(user && user.uid === ADMIN_UID);
  }

  function loginHTML() {
    return `<main class="rp-login-wrap">
      <section class="rp-login-card">
        <div class="rp-login-brand">
          ${logoPerfilHTML("rp-login-logo")}
          <div>
            <h1>Roprost <span>Predict</span></h1>
            <p>Acceso del administrador</p>
          </div>
        </div>
        <form id="rp-login-form" class="rp-login-form" autocomplete="off">
          <label for="rp-user">Usuario</label>
          <input id="rp-user" type="text" placeholder="Usuario" autocomplete="username" required>

          <label for="rp-pass">Contraseña</label>
          <input id="rp-pass" type="password" placeholder="Contraseña" autocomplete="current-password" required>

          <button type="submit" class="rp-login-btn">Ingresar</button>
          <p class="rp-login-note">Acceso protegido con Firebase Authentication.</p>
        </form>
      </section>
    </main>`;
  }
  ...
  function activarLogin() {
    ...
    form.onsubmit = async (e) => {
      ...
      if (!iniciarFirebaseAuth()) {
        alert("Firebase no está cargado. Revisa que index.html tenga los scripts de Firebase.");
        return;
      }

      const usuario = (document.getElementById("rp-user")?.value || "").trim();
      const clave = (document.getElementById("rp-pass")?.value || "").trim();
      const btn = form.querySelector("button[type='submit']");

      const email = usuario.toLowerCase() === ADMIN_USER ? ADMIN_EMAIL : usuario;
      ...
      try {
        const cred = await fbAuth.signInWithEmailAndPassword(email, clave);

        if (!esAdminFirebase(cred.user)) {
          await fbAuth.signOut();
          alert("Este usuario no tiene permiso de administrador.");
          return;
        }
        ...
        alert("Error Firebase: " + (err.code || err.message || "No se pudo iniciar sesión."));
      } finally {
        if (btn) {
          btn.disabled = false;
  ...
  async function cerrarSesion() {
    try {
      if (iniciarFirebaseAuth()) await fbAuth.signOut();
    } catch (e) {
  ...
  function activarCerrarSesion() {
    const btn = document.getElementById("rp-logout-btn");
    if (btn) btn.onclick = cerrarSesion;
  }
  ...
  function logoPerfilHTML(extraClase = "") {
    const foto = perfilFoto();
    const clases = `brand-dot brand-photo-btn${extraClase ? " " + extraClase : ""}${foto ? " has-photo" : ""}`;

    return `<button class="${clases}" id="rp-profile-btn" type="button" title="Cambiar foto de perfil" aria-label="Cambiar foto de perfil">${foto ? `<img class="brand-photo-img" src="${foto}" alt="Foto de perfil">` : ""}</button><input id="rp-profile-input" class="rp-profile-input" type="file" accept="image/*">`;
  }
  ...
  /* ── Layout ───────────────────────────────────────────────────────── */
  function heroHTML() {
    return `<header class="hero"><button id="rp-logout-btn" class="rp-logout-btn" type="button">Cerrar sesión</button><div class="brand">${logoPerfilHTML()}<h1>Roprost <span>Predict</span></h1></div><p class="hero-sub">Análisis selectivo de partidos de hoy y mañana. Pocas apuestas, máxima probabilidad real.</p><p class="hero-tip">Toca el logo para cambiar la foto · Click derecho para quitarla</p></header>`;
  }
  ...
  async function init() {
    ...
    const usuarioActual = await usuarioFirebaseActual();

    if (!esAdminFirebase(usuarioActual)) {
