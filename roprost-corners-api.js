...
const RoprostCornersAPI = (() => {
  ...
  async function buscarFixture(partido) {
    ...
    const encontrado = fixtures.find(fx => {
      ...
      const a = parecido(home, partido.local?.name) && parecido(away, partido.visitante?.name);
      const b = parecido(home, partido.visitante?.name) && parecido(away, partido.local?.name);
